...
def run_inference(session, image_bytes, imgsz=(640, 640)):
    ...
    img, ratio, (dw, dh) = letterbox(image_bytes, new_shape=imgsz)  # 调整图像尺寸
...
def save_detections(image, detections):
    ...
    return image  # 保存绘制后的图像


def process_images_in_folder(img, model_weights, conf_threshold, iou_threshold, imgsz):
    """
    ...
    raw_output, ratio, dwdh = run_inference(session=session, image_bytes=img, imgsz=imgsz)  # 执行推理
    detections = parse_onnx_output(raw_output, ratio, dwdh, conf_threshold=conf_threshold,
                                           iou_threshold=iou_threshold)  # 解析输出

    #im0 = cv2.imdecode(np.frombuffer(img, np.uint8), cv2.IMREAD_COLOR)  # 解码图像
    return save_detections(img, detections)  # 保存检测结果
...
if __name__ == "__main__":
    ...
    process_images_in_folder(folder_path, model_weights, conf_threshold, iou_threshold, imgsz)  # 执行